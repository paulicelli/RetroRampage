...
class CoreAnimationView: UIView {
    ...
    func draw(_ world: World) {
        ...
        // Player weapon
        let weaponTexture = UIImage(named: world.player.animation.texture.rawValue)!
        ...
        // Health
        let font = UIImage(named: Texture.font.rawValue)!
        let charSize = CGSize(width: font.size.width / 10, height: font.size.height)
        ...
        // Weapon icon
        let weaponIcon = UIImage(named: world.player.weapon.attributes.hudIcon.rawValue)!
    ...
    func addLayer(for image: UIImage, xRange: Range<Int>? = nil,
        ...
        if let xRange = xRange {
            let start = CGFloat(xRange.lowerBound) / image.size.width
            let end = CGFloat(xRange.upperBound) / image.size.width
